        ...
        if (enforceWordCount(userMessage, 2)) {
            ...
            try {
                ...
                if (err.includes('Error 2:')) {
                    ...
                    return {
                        responseMessage: `${player.username} is already in queue.`,
                        deleteSenderMessage: false
                    };
                }
                if (err.includes('Error 3:')) {
                    return {
                        responseMessage:
                            'Player is in an unreported match. Please finish the match and wait for both captains to report the score.',
                        deleteSenderMessage: false
                    };
                }
            }
