...
async function queue(msg) {
	...
	if (userMessage.startsWith('!queue leave')) {
        ...
        if (enforceWordCount(userMessage, 2)) {
			...
			for (const [index, id] of queuedPlayers.entries()){
				if (userID === id.discordId) {
					let username = id.Player.username;
					queuedPlayers.splice(index, 1);
					return {
						responseMessage: `${username} has left the queue. Players in queue: ${queuedPlayers.length}`,
        ...
        return {
            responseMessage:
                'Syntax error: To leave the queue make sure to type: !queue leave',
            deleteSenderMessage: false
        };
    }
    ...
    if (userMessage.startsWith('!queue join')) {
        ...
        if (enforceWordCount(userMessage, 2)) {
			...
			try {
				...
				// Pop queue
				if (queuedPlayers.length === 3) {
					queuePop(queuedPlayers);
					return {
						responseMessage: `${player.Player.username} has joined the queue. Players in queue: ${queuedPlayers.length}. Beginning Draft.`,
						deleteSenderMessage: false
					};
				}
				return {
					responseMessage: `${player.Player.username} has joined the queue. Players in queue: ${queuedPlayers.length}.`,
					deleteSenderMessage: false
				};
			} catch (err) {
				if (err.includes("Error 1:")) {
					return {
						responseMessage: `Error: ${msg.author.username} has not registered a username and cannot queue.`,
						deleteSenderMessage: false
					};
				}
        ...
        return {
            responseMessage:
                'Syntax error: To join the queue make sure to type: !queue join',
            deleteSenderMessage: false
        };
    }
