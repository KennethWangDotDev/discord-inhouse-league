...
bot.on('ready', () => {
    ...
    if (!server) {
        ...
        logger.log('error', `Server ${cfg.serverName} not found.`);
        process.exit(-1);
    }
    ...
    logger.log('info', `Bot instance ready! Serving for a total of ${bot.users.size} users!`);
    console.log(cmdChannels[0]);
    cmdChannels[0].send(`Bot is now online! Version: ${version}`);
});
