...
(async function () {
    ...
    const me = {
        discordId: '126134237270114304'
    };
    const response = await associatePlayersToMatches([me], 'cjc6wy5m04ze20156fskn7k6u', 1);
 ...
 */
const updatePlayerRatingsQuery = async (players) => {
    const BATCH_SIZE = 50;
...
const associatePlayersToMatchesQuery = async (players, matchId, win) => {
    ...
    if (win === 0) {
        ...
        await associatePlayersToMatches(players, matchId, 0);
    } else {
...
const playerMatchRatingsChangeQuery = async (players, matchId, ratingsChangeObject) => {
    ...
    const mutations = _.chain(players)
        .map(player => `
            k_${player.discordId}: PlayerMatchRatingsChange (
